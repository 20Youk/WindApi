...
def send_email(from_addr, to_addr, subject, password):
    ...
    cursor = conn.cursor()
    sql1 = '''select * from AppLog where convert(date,logdate,20)=convert(date,getdate() ,20) and AppSource='ImportWind' and DATEPART(HH, LogDate) <= 8 order by LogDate'''
...
if __name__ == "__main__":
    if send_email(u"palh8888@163.com", u"jyyouzi@163.com", u"Wind_Api晨间任务执行报告", u"wind1234"):
        print '邮件发送成功！'
    else:
